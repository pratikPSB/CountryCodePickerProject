...
class InternationalPhoneTextWatcher @JvmOverloads constructor(
    ...
    private lateinit var mFormatter: AsYouTypeFormatter
    ...
    private fun reformat(s: CharSequence): String {
        ...
        if (internationalOnly || s.length > 0 && s[0] != '0') //to have number formatted as international format, add country code before that
        ...
        if (internationalOnly || s.length == 0 || s[0] != '0') {
            internationalFormatted = if (internationalFormatted.length > countryCallingCode.length) {
